let student_form = document.getElementById('mod_grade_form');
...
student_form.addEventListener('submit', async (e) => {
    ...
    if (student_form.checkValidity() === true) {
        let response = await fetch('api/grades/update', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({
                ...
                letter_grade: document.getElementById('newLetter').value,
                grade_points: document.getElementById('newGradePoint').value,
                comment: document.getElementById('newComment').value,
            })
        }).then(
            response => {
                if(response['status'] === 200){
                    alert("Grade Details updated successfully !!!");
                    location.reload();
                }else{
